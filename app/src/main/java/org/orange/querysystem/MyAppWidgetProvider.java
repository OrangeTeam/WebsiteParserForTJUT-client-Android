...
public class MyAppWidgetProvider extends AppWidgetProvider {
    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        ...
        alarmManager.setRepeating(AlarmManager.ELAPSED_REALTIME, triggerAtTime, interval,
                pendingIntent);
    ...
    public void updateCourse(Context context, AppWidgetManager appWidgetManager,
            int[] appWidgetIds) {
        ...
        try {
            ...
            courses = studentInfDBAdapter.getCoursesFromDB(
                    StudentInfDBAdapter.KEY_YEAR + "= ?  AND " + StudentInfDBAdapter.KEY_SEMESTER
                            + " = ?",
                    new String[]{String.valueOf(year), String.valueOf(semester)}, null,
                    SettingsActivity.getAccountStudentID(context)
            );
        ...
        for (Course course : courses) {
            for (Course.TimeAndAddress time : course.getTimeAndAddress()) {
                try {
                    if (time.hasSetDay(mDayOfWeek) && time.hasSetPeriod(thePeriod) && time
                            .hasSetWeek(SettingsActivity.getCurrentWeekNumber(context) + 1)) {
                        lesson[mDayOfWeek][thePeriod].addLast(
                                new SimpleCourse(course.getId(), course.getName(),
    ...
    private void showResult(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds,
            String str) {
            ...
            if (((mHour == 8) && (mMinute > 30)) || ((mHour == 9) && (mMinute <= 25))) {
                ...
                if (((mHour == 9) && (mMinute > 25)) || ((mHour == 10) && (mMinute <= 40))) {
                    ...
                    if (((mHour == 10) && (mMinute > 40)) || ((mHour == 11) && (mMinute <= 35))) {
                        ...
                        if (((mHour == 11) && (mMinute > 35)) || ((mHour == 12) && (mMinute
                                <= 45))) {
                            period = 5;
                        } else {
                            if (((mHour == 12) && (mMinute > 45)) || (mHour == 13) || ((mHour == 14)
                                    && (mMinute <= 30))) {
                                period = 7;
                            } else {
                                if (((mHour == 14) && (mMinute > 30)) || ((mHour == 15) && (mMinute
                                        <= 25))) {
                                    period = 8;
                                } else {
                                    if (((mHour == 15) && (mMinute > 25)) || ((mHour == 16) && (
                                            mMinute <= 40))) {
                                        period = 9;
                                    } else {
                                        if (((mHour == 16) && (mMinute > 40)) || ((mHour == 17) && (
                                                mMinute <= 35))) {
                                            period = 10;
                                        } else {
                                            if (((mHour == 17) && (mMinute > 35)) || (mHour
                                                    == 18)) {
                                                period = 11;
                                            } else {
