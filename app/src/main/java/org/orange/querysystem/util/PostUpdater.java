...
public class PostUpdater {
    ...
    public boolean updatePosts(boolean mandatorily) {
        ...
        if (!mandatorily) {
            ...
            if (updatedTime != null
                    && System.currentTimeMillis() - updatedTime.getTime() < updateInterval) {
                return false;
            }
        }
    ...
    private class UpdatePostsListToDatabase extends AsyncTask<Void, Void, Long> {
        ...
        private Date getTimeOfLatestPost() {
            ...
            try {
                ...
                Post lastPost = database
                        .getPostsFromDB(null, Contract.Posts.COLUMN_NAME_DATE + " DESC", "1")
                        .get(0);
        ...
        private void refreshLastUpdatedTime(long updateInterval) {
            ...
            if (updatedTime == null
                ...
                if ((updatedTime == null) || (lastPostTime != null
                        && lastPostTime.compareTo(updatedTime) > 0)) {
            ...
            for (int counter = 1; counter <= maxAttempts; counter++) {
                try {
                    ...
                    if (e.getCause() instanceof SocketTimeoutException) {
                        Log.i(TAG,
                                "用Hessian连接GAE代理更新通知时，遇到SocketTimeoutException" + e.getMessage());
                        continue;
                    } else {
                        e.printStackTrace();
                    }
